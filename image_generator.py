...
def search_pexels_image(query, output_path):
    ...
    try:
        ...
        query_words = query.lower().replace("image of", "").replace("photo of", "").replace("picture of", "")
        query_words = query_words.replace("a ", "").replace("an ", "").replace("the ", "")
        # Take first few meaningful words
        clean_query = " ".join(query_words.split()[:3])
        
        logging.info(f"Searching Pexels for: '{clean_query}' (from prompt: '{query[:50]}...')")
...
def generate_images(prompts, output_dir):
    ...
    for idx, prompt in enumerate(prompts, start=1):
        ...
        try:
            logging.info(f"🎨 Attempting OpenAI DALL-E for image {idx}/{len(prompts)}: {prompt[:50]}...")
            ...
            logging.warning(f"⚠️ OpenAI DALL-E failed for image {idx}/{len(prompts)}: {str(openai_error)}")
            
            # Strategy 2: Fall back to Pexels stock images
            try:
                logging.info(f"🔄 Falling back to Pexels for image {idx}/{len(prompts)}: {prompt[:50]}...")
                image_path = search_pexels_image(prompt, output_path)
                image_paths.append(image_path)
                logging.info(f"✅ Pexels fallback success for image {idx}/{len(prompts)}")
                image_generated = True
            except Exception as pexels_error:
                logging.error(f"❌ Both OpenAI and Pexels failed for image {idx}/{len(prompts)}")
    ...
    if len(image_paths) == 0:
        ...
        raise Exception(f"Failed to generate any images. All {len(prompts)} image generation attempts failed with both OpenAI and Pexels.")
    
    success_rate = len(image_paths) / len(prompts) * 100
    logging.info(f"✅ Successfully generated {len(image_paths)}/{len(prompts)} images ({success_rate:.1f}% success rate)")
    return image_paths
