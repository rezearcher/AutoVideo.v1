...
def test_vertex_ai_init():
    ...
    try:
        ...
        try:
            ...
            operation_time = time.time() - start_time
            logger.warning(f"⚠️ Client operation failed in {operation_time:.2f}s: {op_error}")
            logger.warning("This might be normal if no jobs exist or permissions are limited")
    ...
    for endpoint in endpoints:
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(f"⚠️ DNS resolution failed for {endpoint}: {result.stderr}")
...
if __name__ == "__main__":
    ...
    if success:
        logger.info("🎯 All tests passed! Vertex AI should work in the main application.")
        sys.exit(0)
    else:
