...
class TopicManager:
    def __init__(
        self, topics_file="topics.json", max_topics=10, update_interval_days=7
    ):
            ...
            try:
                with open(self.topics_file, "r") as f:
                    ...
                    self.last_update = datetime.fromisoformat(
                        data.get("last_update", datetime.min.isoformat())
                    )
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a creative writing assistant. Generate unique and engaging story prompts. NEVER include prompts about bookstores, libraries, or books coming to life. Focus on diverse themes like adventure, mystery, science fiction, fantasy, and drama.",
                    },
                    {
                        "role": "user",
                        "content": f"Generate {self.max_topics} unique and creative story prompts. Each prompt should start with 'Write a story about'. Make them diverse and interesting. DO NOT include any prompts about bookstores, libraries, or books.",
                    },
                ],
                max_tokens=500,
                temperature=0.9,
            ...
            for line in content.split("\n"):
                if line.strip().startswith("Write a story about"):
                    ...
                    if not any(
                        word in line.lower()
                        for word in ["book", "library", "bookstore"]
                    ):
    ...
    def get_next_topic(self):
        ...
        if not self.topics:
            ...
            if not self.used_topics:
                ...
                self.topics = [
                    topic for topic in self.used_topics[:-1] if topic != last_used
                ]
                ...
                if not self.topics:
                    logger.warning(
                        "Still no topics after recycling, generating new ones..."
                    )
                    self._generate_new_topics()
