...
class MonitoringSetup:
    ...
    def create_notification_channels(self, channels_config: List[Dict]) -> Dict[str, str]:
            ...
            try:
                ...
                for existing in existing_channels:
                    if existing.display_name == channel_config['name']:
                        ...
                        channel_exists = True
                        print(f"Notification channel '{channel_config['name']}' already exists")
                        break
                
                if not channel_exists:
                    # Create new notification channel
                    channel = monitoring_v3.NotificationChannel(
                        type_=f"email" if channel_config['type'] == 'email' else f"slack",
                ...
                print(f"Error creating notification channel '{channel_config['name']}': {e}")
                
        return channel_map

    def create_alerting_policies(self, policies_config: List[Dict], notification_channels: Dict[str, str]):
            ...
            try:
                ...
                for existing in existing_policies:
                    if existing.display_name == policy_config['display_name']:
                        policy_exists = True
                        print(f"Alert policy '{policy_config['display_name']}' already exists")
                        break
                        ...
                        condition = monitoring_v3.AlertPolicy.Condition(
                            ...
                            condition_threshold=monitoring_v3.AlertPolicy.Condition.MetricThreshold(
                                ...
                                comparison=getattr(
                                    ...
                                    condition_config['condition_threshold']['comparison']
                                ),
                                threshold_value=condition_config['condition_threshold']['threshold_value'],
                                duration={"seconds": int(condition_config['condition_threshold']['duration'][:-1])},
                                aggregations=[
                                    monitoring_v3.Aggregation(
                                        alignment_period={"seconds": int(agg['alignment_period'][:-1])},
                ...
                print(f"Error creating alert policy '{policy_config['display_name']}': {e}")
    ...
    def create_custom_metrics(self, metrics_config: List[Dict]):
        ...
        for metric_config in metrics_config:
            try:
                ...
                try:
                    existing_metric = self.metric_client.get_metric_descriptor(name=metric_name)
                ...
                descriptor = monitoring_v3.MetricDescriptor(
                    ...
                    metric_kind=getattr(monitoring_v3.MetricDescriptor.MetricKind, metric_config['type']),
    ...
    def create_dashboard(self, dashboard_config: Dict):
        ...
        try:
            ...
            existing_dashboards = self.dashboard_client.list_dashboards(parent=self.project_name)
    ...
    def setup_monitoring(self, config_file: str):
        ...
        if 'notification_channels' in config:
            notification_channels = self.create_notification_channels(config['notification_channels'])
        ...
        if 'alerting_policies' in config:
            self.create_alerting_policies(config['alerting_policies'], notification_channels)
...
def main():
    parser = argparse.ArgumentParser(description='Setup Google Cloud Monitoring for AutoVideo')
    parser.add_argument('--project-id', required=True, help='Google Cloud Project ID')
    parser.add_argument('--config', default='monitoring-config.yaml', help='Monitoring configuration file')
