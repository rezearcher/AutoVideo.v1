...
class PreemptibleGPUManager:
    ...
    def _track_preemptible_job(
        self, job_id: str, image_paths: list, audio_path: str, story: str
    ):
    ...
    def wait_for_job_with_retry(
        ...
        while attempt < self.max_retries:
            try:
                logger.info(
                    f"Waiting for job {job_id} (attempt {attempt+1}/{self.max_retries})"
                )
                ...
                if self._was_job_preempted(result):
                    ...
                    if attempt < self.max_retries:
                        ...
                        result = self.vertex_service.wait_for_job_completion(
                            job_id, timeout
                        )
                        return result
                else:
    ...
    def _retry_with_non_preemptible(self, original_job_id: str) -> str:
        ...
        logger.info(
            f"Falling back to non-preemptible instance for job {original_job_id}"
        )
    ...
    def modify_vertex_configs_for_preemptible(self):
        ...
        logger.info(
            f"Added preemptible configurations. Total configs: {len(new_configs)}"
        )
...
if __name__ == "__main__":
    ...
    manager = PreemptibleGPUManager(
        project_id=project_id, region="us-central1", max_retries=3, retry_delay=30
    )
    ...
    logger.info(
        "Use manager.create_video_job_with_retry() to create jobs with preemptible GPU support"
    )
