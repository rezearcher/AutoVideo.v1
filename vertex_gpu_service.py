...
def initialize_vertex_ai(project_id: str, region: str, staging_bucket: str):
    ...
    if not _vertex_initialized:
        logger.info(
            f"🚀 Initializing Vertex AI globally: project={project_id}, region={region}"
        )
        aiplatform.init(
...
def get_gpu_quota(
    project: str, region: str, gpu_type: str = "T4"
    ...
    try:
        ...
        metric_map = {"T4": "NVIDIA_T4_GPUS", "L4": "NVIDIA_L4_GPUS"}
...
def get_multi_region_quota_status(
    project_id: str, regions: List[str] = None
...
def discover_gpu_machine_compatibility(project_id: str, region: str) -> Dict[str, str]:
    ...
    try:
        ...
        zones_result = (
            compute.zones()
            .list(project=project_id, filter=f"region eq .*{region}")
            .execute()
        )
        ...
        accelerators = (
            compute.acceleratorTypes().list(project=project_id, zone=zone).execute()
        )
        available_gpus = set()
        ...
        machine_types = (
            compute.machineTypes().list(project=project_id, zone=zone).execute()
        )
        ...
        if "NVIDIA_L4" in available_gpus:
            if "g2-standard-8" in available_machines:
                ...
                logger.warning(
                    f"L4 GPU available in {region} but no compatible machine type found"
                )
        ...
        if "NVIDIA_TESLA_T4" in available_gpus:
            if "n1-standard-4" in available_machines:
                ...
                logger.warning(
                    f"T4 GPU available in {region} but no compatible machine type found"
                )
...
def get_machine_type_for_gpu(
    region: str, gpu_type: str, project_id: str = None
) -> Optional[str]:
    ...
    if region in REGION_GPU_MACHINE_MAP:
        ...
        if machine_type:
            logger.info(
                f"Using static mapping: {gpu_type} -> {machine_type} in {region}"
            )
            return machine_type
...
class VertexGPUJobService:
    def __init__(
        self, project_id: str, region: str = "us-central1", bucket_name: str = None
    ):
        ...
        if override_region:
            logger.info(
                f"🌍 Region override detected: {override_region} (from DEFAULT_REGION env var)"
            )
    ...
    def _generate_fallback_configs(self) -> List[Dict[str, Any]]:
        ...
        for region in regions:
            for gpu_type in gpu_types:
                vertex_gpu_type = (
                    f"NVIDIA_{gpu_type}" if gpu_type == "L4" else "NVIDIA_TESLA_T4"
                )
                machine_type = get_machine_type_for_gpu(
                    region, vertex_gpu_type, self.project_id
                )
                ...
                if machine_type:
                    ...
                    logger.info(
                        f"✅ Added fallback: {gpu_type} on {machine_type} in {region}"
                    )
                else:
                    logger.warning(
                        f"⚠️ Skipped {gpu_type} in {region} - no compatible machine type"
                    )
    ...
    def get_best_available_config(self) -> Dict[str, Any]:
        ...
        for i, config in enumerate(self.fallback_configs):
            ...
            if gpu_type is None:  # CPU-only fallback
                logger.info(
                    f"✅ Fallback {i+1}: CPU-only ({machine_type}) in {region}{spot_label}"
                )
            ...
            if quota_info and quota_info.get("available", 0) > 0:
                logger.info(
                    f"✅ Fallback {i+1}: {gpu_type} available in {region}{spot_label} (quota: {quota_info})"
                )
                return config
            else:
                quota_msg = (
                    f"quota: {quota_info}" if quota_info else "quota check failed"
                )
                logger.warning(
                    f"❌ Fallback {i+1}: {gpu_type} unavailable in {region}{spot_label} ({quota_msg})"
                )
    ...
    def upload_assets_to_gcs(
        self, job_id: str, image_paths: List[str], audio_path: str
    ) -> Dict[str, Any]:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt == max_retries - 1:
                                logger.error(
                                    f"Failed to upload image {i} after {max_retries} attempts: {e}"
                                )
                                raise
                            logger.warning(
                                f"Upload attempt {attempt + 1} failed for image {i}, retrying: {e}"
                            )
            ...
            if os.path.exists(audio_path):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt == max_retries - 1:
                            logger.error(
                                f"Failed to upload audio after {max_retries} attempts: {e}"
                            )
                            raise
                        logger.warning(
                            f"Upload attempt {attempt + 1} failed for audio, retrying: {e}"
                        )
                        time.sleep(2**attempt)  # Exponential backoff
    ...
    def _submit_job_with_config(
        self, job_id: str, config: Dict[str, Any], job_config_url: str
    ) -> str:
        ...
        if region != self.primary_region:
            ...
            if "central1" in self.bucket_name:
                regional_bucket_name = self.bucket_name.replace(
                    "central1", region.replace("us-", "")
                )
            else:
                regional_bucket_name = (
                    f"{self.project_id}-vertex-staging-{region.replace('us-', '')}"
                )
            logger.info(
                f"🌍 Using regional bucket for {region}: {regional_bucket_name}"
            )
        ...
        try:
            logger.info(
                f"🚀 Submitting job to {region} with {gpu_type or 'CPU'}{spot_label}"
            )
            ...
            worker_pool_spec = {
                ...
                "container_spec": {
                    ...
                    "command": [
                        ...
                        self.project_id,
                        "--bucket-name",
                        self.bucket_name,  # Use original bucket for data access
                        "--config-url",
                        job_config_url,
                    ],
                    "env": [
            ...
            if gpu_type and gpu_count > 0:
                ...
                gpu_type_map = {"T4": "NVIDIA_TESLA_T4", "L4": "NVIDIA_L4"}

                worker_pool_spec["machine_spec"]["accelerator_type"] = gpu_type_map[
                    gpu_type
                ]
            ...
            if region != self.primary_region:
                logger.info(
                    f"🏠 Switching back to primary region {self.primary_region}"
                )
            ...
            error_str = str(e)
            logger.error(
                f"❌ Job submission failed in {region}{spot_label}: {error_str}"
            )
            ...
            if region != self.primary_region:
                try:
                    logger.info(
                        f"🏠 Switching back to primary region {self.primary_region} after error"
                    )
                    primary_staging_bucket = f"gs://{self.bucket_name}"
                    aiplatform.init(
                        project=self.project_id,
                    ...
                    logger.warning(
                        f"⚠️ Failed to switch back to primary region: {init_error}"
                    )
            ...
            if (
                ...
                if (
                    "machine type" in error_str.lower()
                    or "not supported" in error_str.lower()
                ):
                    logger.warning(
    ...
    def create_video_job(
        self, image_paths: List[str], audio_path: str, story: str
    ) -> str:
