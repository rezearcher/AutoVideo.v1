...
class TTSService:
    ...
    def generate_speech(self, 
        ...
        text_hash = hashlib.sha256(f"{text}_{voice}_{language_code}".encode()).hexdigest()[:12]
        ...
        if self._elevenlabs_available:
            try:
                ...
                logger.warning(f"ElevenLabs TTS failed - falling back to Google TTS: {e}")
    ...
    def _normalize_audio(self, audio_path: str) -> None:
        ...
        try:
            normalize_audio_loudness(audio_path, audio_path, target_loudness=-16, 
                                     true_peak=-1.5, loudness_range=11)
