...
def create_render_script():
    """Create the Python render script content"""
...
cat > /workspace/render_job.py << 'RENDER_SCRIPT_EOF'
...
def launch_vm(gpu_config, gpu_key, vm_name, project_id, zone="us-central1-a"):
    ...
    startup_script = create_startup_script(
        project_id, staging_bucket, output_bucket, job_id
    )
...
def main():
    ...
    if success:
        ...
        print(
            f"   Monitor with: gcloud compute instances list --filter='name:{vm_name}'"
        )
        print(
            f"   Logs: gcloud compute instances get-serial-port-output {vm_name} --zone={zone}"
        )
    else:
