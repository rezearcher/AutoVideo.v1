...
def parse_args():
    ...
    parser.add_argument(
        "--topic", type=str, required=True, help="Topic to generate a video about"
    )
...
def generate_video_with_preemptible_gpus(
    topic, use_preemptible=True, max_retries=5, retry_delay=30, region="us-central1"
):
    """
    ...
    logger.info(
        f"🚀 Generating video about '{topic}' with {'preemptible' if use_preemptible else 'standard'} GPUs"
    )
    ...
    try:
        ...
        logger.info(
            f"⏳ Waiting for job {job_id} to complete (with automatic retry if preempted)..."
        )
...
def main():
    ...
    logger.info(
        f"Starting AutoVideo with {'preemptible' if args.preemptible else 'standard'} GPUs"
    )
